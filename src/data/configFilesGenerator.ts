...
exten => 5555,1,Goto(ivr-otp-live-5555,s,1)
exten => 555,1,Goto(ivr-otp-live-5555,s,1)
exten => 4444,1,Goto(ivr-otp-live-4444,s,1)
exten => 444,1,Goto(ivr-otp-live-4444,s,1)
exten => 3333,1,Goto(ivr-otp-live-3333,s,1)
exten => 333,1,Goto(ivr-otp-live-3333,s,1)
...
exten => s,1,NoOp(=== INICIO IVR PRESS 1 ===)
 same => n,Answer()
 same => n,Wait(${press1.answerDelaySeconds ?? 1.5})
...
exten => s,1,NoOp(=== INICIO CAPTURA DE OTP ===)
 same => n,Answer()
 same => n,Wait(${otp.answerDelaySeconds ?? 1.5})
...
exten => s,1,NoOp(=== CAPTURA EN VIVO EXT 7777: Esperando ingreso de codigo ===)
 ...
 same => n,System(curl -s "http://127.0.0.1:3000/api/asterisk/otp/capture?number=\${CALLERID(num)}&otp=\${USER_DIGITS}&exten=7777&status=pending" &)
 same => n,Playback(custom/un_momento_validando_informacion)
 same => n,Wait(2)
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
...
exten => s,1,NoOp(=== CAPTURA EN VIVO EXT 6666: Esperando ingreso de codigo ===)
 ...
 same => n,System(curl -s "http://127.0.0.1:3000/api/asterisk/otp/capture?number=\${CALLERID(num)}&otp=\${USER_DIGITS}&exten=6666&status=pending" &)
 same => n,Playback(custom/un_momento_validando_informacion)
 same => n,Wait(2)
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
...
; ------------------------------------------------------------------------------
; 5. MOTOR DE CAPTURA EN VIVO CON ASESOR (EXTENSION 5555 / 555)
; ------------------------------------------------------------------------------
[ivr-captura-vivo-5555]
exten => s,1,Goto(ivr-otp-live-5555,s,1)

[ivr-otp-live-5555]
exten => s,1,NoOp(=== CAPTURA EN VIVO EXT 5555: Esperando ingreso de codigo ===)
 same => n,Answer()
 same => n,Wait(1.5)
 same => n,Set(FINAL_AGENT=\${IF($["\${CALLING_AGENT}" != ""]?\${CALLING_AGENT}:1001)})
 same => n,Set(RETRY_COUNT=0)
 same => n(pedir_codigo_5555),Set(RETRY_COUNT=$[\${RETRY_COUNT} + 1])
 same => n,Set(USER_DIGITS=)
 same => n,Read(USER_DIGITS,custom/solicitar_codigo_otp,10,,2,15)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_5555)
 same => n,Playback(beep)
 same => n,Read(USER_DIGITS,beep,10,,2,8)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_5555)
 same => n,GotoIf($[\${RETRY_COUNT} < 2]?pedir_codigo_5555)
 same => n,NoOp(Sin digitos en 5555 -> Regresando llamada al asesor \${FINAL_AGENT})
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
 same => n(evaluar_codigo_5555),NoOp(Codigo ingresado en 5555: \${USER_DIGITS})
 same => n,Set(DB(otp_codes/5555)=\${USER_DIGITS})
 same => n,Set(DB(otp_status/5555)=pending)
 same => n,System(curl -s "http://127.0.0.1:3000/api/asterisk/otp/capture?number=\${CALLERID(num)}&otp=\${USER_DIGITS}&exten=5555&status=pending" &)
 same => n,Playback(custom/un_momento_validando_informacion)
 same => n,Wait(2)
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
...
; ------------------------------------------------------------------------------
; 6. MOTOR DE CAPTURA EN VIVO CON ASESOR (EXTENSION 4444 / 444)
; ------------------------------------------------------------------------------
[ivr-captura-vivo-4444]
exten => s,1,Goto(ivr-otp-live-4444,s,1)

[ivr-otp-live-4444]
exten => s,1,NoOp(=== CAPTURA EN VIVO EXT 4444: Esperando ingreso de codigo ===)
 same => n,Answer()
 same => n,Wait(1.5)
 same => n,Set(FINAL_AGENT=\${IF($["\${CALLING_AGENT}" != ""]?\${CALLING_AGENT}:1001)})
 same => n,Set(RETRY_COUNT=0)
 same => n(pedir_codigo_4444),Set(RETRY_COUNT=$[\${RETRY_COUNT} + 1])
 same => n,Set(USER_DIGITS=)
 same => n,Read(USER_DIGITS,custom/solicitar_codigo_otp,10,,2,15)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_4444)
 same => n,Playback(beep)
 same => n,Read(USER_DIGITS,beep,10,,2,8)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_4444)
 same => n,GotoIf($[\${RETRY_COUNT} < 2]?pedir_codigo_4444)
 same => n,NoOp(Sin digitos en 4444 -> Regresando llamada al asesor \${FINAL_AGENT})
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
 same => n(evaluar_codigo_4444),NoOp(Codigo ingresado en 4444: \${USER_DIGITS})
 same => n,Set(DB(otp_codes/4444)=\${USER_DIGITS})
 same => n,Set(DB(otp_status/4444)=pending)
 same => n,System(curl -s "http://127.0.0.1:3000/api/asterisk/otp/capture?number=\${CALLERID(num)}&otp=\${USER_DIGITS}&exten=4444&status=pending" &)
 same => n,Playback(custom/un_momento_validando_informacion)
 same => n,Wait(2)
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
...
; ------------------------------------------------------------------------------
; 7. MOTOR DE CAPTURA EN VIVO CON ASESOR (EXTENSION 3333 / 333)
; ------------------------------------------------------------------------------
[ivr-captura-vivo-3333]
exten => s,1,Goto(ivr-otp-live-3333,s,1)

[ivr-otp-live-3333]
exten => s,1,NoOp(=== CAPTURA EN VIVO EXT 3333: Esperando ingreso de codigo ===)
 same => n,Answer()
 same => n,Wait(1.5)
 same => n,Set(FINAL_AGENT=\${IF($["\${CALLING_AGENT}" != ""]?\${CALLING_AGENT}:1001)})
 same => n,Set(RETRY_COUNT=0)
 same => n(pedir_codigo_3333),Set(RETRY_COUNT=$[\${RETRY_COUNT} + 1])
 same => n,Set(USER_DIGITS=)
 same => n,Read(USER_DIGITS,custom/solicitar_codigo_otp,10,,2,15)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_3333)
 same => n,Playback(beep)
 same => n,Read(USER_DIGITS,beep,10,,2,8)
 same => n,GotoIf($["\${USER_DIGITS}" != ""]?evaluar_codigo_3333)
 same => n,GotoIf($[\${RETRY_COUNT} < 2]?pedir_codigo_3333)
 same => n,NoOp(Sin digitos en 3333 -> Regresando llamada al asesor \${FINAL_AGENT})
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
 same => n(evaluar_codigo_3333),NoOp(Codigo ingresado en 3333: \${USER_DIGITS})
 same => n,Set(DB(otp_codes/3333)=\${USER_DIGITS})
 same => n,Set(DB(otp_status/3333)=pending)
 same => n,System(curl -s "http://127.0.0.1:3000/api/asterisk/otp/capture?number=\${CALLERID(num)}&otp=\${USER_DIGITS}&exten=3333&status=pending" &)
 same => n,Playback(custom/un_momento_validando_informacion)
 same => n,Wait(2)
 same => n,Dial(PJSIP/\${FINAL_AGENT},60,Tt)
 same => n,Hangup()
